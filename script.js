...
async function displayAlbums() {
    try {
        ...
        let anchors = Array.from(tempDiv.getElementsByTagName("a"));
        let cardContainer = document.querySelector(".cardContainer");

        for (let e of anchors) {
            if (e.href.includes("/songs/")) {
                let folder = new URL(e.href).pathname.split("/").filter(Boolean).pop();
                if (!folder || folder === "songs" || folder.startsWith(".")) continue;

                try {
                    let res = await fetch(`http://127.0.0.1:3000/songs/${folder}/info.json`);
                    if (!res.ok || !res.headers.get("Content-Type")?.includes("application/json")) {
                        throw new Error(`Invalid JSON or missing info.json for ${folder}`);
                    }
                    ...
                    card.innerHTML = `
                        <div class="play">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                                width="100%" height="100%" color="#000000" fill="none">
